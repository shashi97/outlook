...
export class IndexComponent implements OnInit {
  ...
  ngOnInit() {
    this.jsonApiService.fetch('/campaigns').subscribe(data=> {
      this.temp = [...data];
      this.rows = data;
      this.selected = [];
   ...
   updateFilter(event) {
    ...
    const temp = this.temp.filter(function(d) {
      return !val || ['campaignName','lastName','firstName','title','mobileNumber'].some((field: any)=>{
  ...
  onSelect(event) {
    this.router.navigateByUrl('/associates/details/' + this.selected[0].id);
  }
