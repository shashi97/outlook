...
export class IndexComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.jsonApiService.fetch('/agents/roster').subscribe(data=> {
      this.temp = [...data];
      this.rows = data;
      this.selected = [];
   ...
   updateFilter(event) {
    ...
    const temp = this.temp.filter(function(d) {
      return !val || ['fullName','city','brokerageName'].some((field: any)=>{
  ...
  onSelect(event) {
    this.router.navigateByUrl('/agents/details/' + this.selected[0].id);
  }
